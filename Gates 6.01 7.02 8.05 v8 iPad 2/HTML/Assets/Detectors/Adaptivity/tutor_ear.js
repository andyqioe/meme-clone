	...
	timerID = setTimeout(function() { 
		...
		if(e.data.tool_data.action == "UpdateVariable"){
			...
			if(currentDetectorValues["idle"].split(',')[0]=="1"){
				...
				var messageContent = "insert custom message or interface event for IDLE";
				...
				if(currentDetectorValues["help_model_variant"].indexOf("true, abusing hints?")!=-1){
					var updateType = "write_text";
					var messageContent = "insert custom message or interface event for HINT ABUSE";
					sendTutorMessage(updateType, messageContent);
				}
				else if(currentDetectorValues["help_model_variant"].indexOf("true, avoiding hints?")!=-1){
					var updateType = "write_text";
					var messageContent = "insert custom message or interface event for HINT AVOIDANCE";
					sendTutorMessage(updateType, messageContent);
				}
				else{
					var updateType = "write_text";
					var messageContent = "insert custom message or interface event for GOOD ATTEMPT";
					sendTutorMessage(updateType, messageContent);
				}
