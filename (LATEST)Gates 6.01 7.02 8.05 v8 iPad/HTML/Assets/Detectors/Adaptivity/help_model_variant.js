...
var variableName = "help_model_variant"
...
function is_avoiding_hints(help_model_output){
	if (help_model_output == "hint avoidance"){
		return true;
	}
	else{
		return false;
...
function receive_transaction( e ){
	...
	if(e.data.actor == 'student' && e.data.tool_data.selection !="done" && e.data.tool_data.action != "UpdateVariable"){
		...
		var isAvoidingHints = is_avoiding_hints(help_model_output);
		...
		attemptWindow.shift();
		attemptWindow.push( (isAbusingHints || isNotDeliberate || isAvoidingHints) ? 1 : 0 );
	...
	if(e.data.actor == 'student' && e.data.tool_data.selection !="done" && e.data.tool_data.action != "UpdateVariable"){
		...
		else if(isAvoidingHints){lastTrigger="isAvoidingHints"}
		...
		if (sumAskTeacherForHelp>=threshold){
			...
			else if (lastTrigger=="isAvoidingHints"){
				elaborationString = "avoiding hints?";
			}
			else{
				elaborationString = "fast attempts... not deliberate?";
			}
		}
		else{
			elaborationString = " ";
		...
		if (detector_output.value.split(',')[0]=="false" && (sumAskTeacherForHelp >= threshold)){
			initTime = new Date();
			detector_output.history = JSON.stringify([attemptWindow, initTime, lastTrigger, onboardSkills, attemptCount, abuseCount, avoidanceCount]);
			detector_output.value = "true, " + elaborationString + "," + "hint abuse: " + (abuseCount/attemptCount).toString() + "%<br> hint avoidance: " + (avoidanceCount/attemptCount).toString() + "%";
			detector_output.time = new Date();

			//intervalID = setInterval( function() { checkTimeElapsed(initTime);} , 3000);

		}
		else if (detector_output.value.split(',')[0]!="false" && (sumAskTeacherForHelp >= threshold)){
			detector_output.history = JSON.stringify([attemptWindow, initTime, lastTrigger, onboardSkills, attemptCount, abuseCount, avoidanceCount]);
			detector_output.value = "true, " + elaborationString + "," + "hint abuse: " + (abuseCount/attemptCount).toString() + "%<br> hint avoidance: " + (avoidanceCount/attemptCount).toString() + "%";
			detector_output.time = new Date();
		}
		else{
			detector_output.value = "false, " + " ," + "hint abuse: " + (abuseCount/attemptCount).toString() + "%<br> hint avoidance: " + (avoidanceCount/attemptCount).toString() + "%";
			detector_output.history = JSON.stringify([attemptWindow, initTime, lastTrigger, onboardSkills, attemptCount, abuseCount, avoidanceCount]);
			detector_output.time = new Date();
        ...
        //the detector_output_ear is sth that send it to tutor_ear
...
self.onmessage = function ( e ) {
	...
	case "initialize":
		...
		if (detector_output.history == "" || detector_output.history == null){
			...
			attemptCount = all_history[4];
			abuseCount = all_history[5];
			avoidanceCount = all_history[6];
